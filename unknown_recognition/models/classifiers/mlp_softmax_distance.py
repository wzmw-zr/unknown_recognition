...
class MLPSoftmaxDistance(BaseModule):
    def __init__(
        self, 
        in_channels=1,
        hidden_channels=256,
        ...
        super(MLPSoftmaxDistance, self).__init__(init_cfg)
    ...
    def forward_train(self, softmax_distance: Tensor, img_metas, gt_semantic_seg):
        seg_logits = self.forward(softmax_distance)
        loss = self.losses(seg_logits, gt_semantic_seg)
        return loss

    def forward_test(self, softmax_distance: Tensor, img_metas):
        return self.forward(softmax_distance)
